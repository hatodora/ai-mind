...
- 公開モーダルは実際に投稿されるツリーを読み取り専用マップとしてプレビュー表示し、
  **タイトル・本文を自由入力できる（両方とも任意・空でも投稿可）**（2026-07-18 追加）。
  タイトル未入力時は表示で rootLabel（起点ノードのラベル）を使う。
...
posts/{postId}:
  ...
  rootLabel（選択ノードのラベル）,
  title?（任意・≤100字。未入力なら省略）, body?（任意・≤1000字。未入力なら省略）,
  nodes[{id,label,role,position}] (≤200),
